    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="p-6">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-bold text-gray-900">Create Payment for Purchase #{{ $purchase->purchase_number }}</h2>
            ...
            <form action="{{ route('vendor-payments.store-from-purchase') }}" method="POST">
                @csrf
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-sm font-medium">{{ $purchase->vendor->name }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-600">Amount to be Paid:</p>
                                <p class="text-sm font-medium">Rs. {{ number_format($purchase->net_amount, 2) }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-600">Invoice Date:</p>
                                <p class="text-sm font-medium">{{ date('d-m-Y', strtotime($purchase->purchase_date)) }}</p>
                            </div>
                            <div>
