...
<head>
    ...
    <title>Inventory Ledger - {{ $inventory->name }}</title>
...
<body>
    ...
    <div class="inventory-info" style="margin-bottom: 15px; text-align: right;">
        <div style="font-size: 14px; font-weight: bold;">Inventory Ledger</div>
        ...
        <tbody>
            @foreach($rows as $row)
                <tr @if(strpos($row['party'] ?? '', 'Opening') !== false) class="opening-row" @endif>
                    ...
                    <td class="text-right">{{ isset($row['rate']) && $row['rate'] !== null ? number_format($row['rate'], 0) : '' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
